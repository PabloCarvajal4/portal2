...
export class CambioPasswordComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    this.credencialForm = this.fb.group({
      //codigo_colaborador: [this.colaboradorId, Validators.required],
  ...
  agregarCredencial() {
    ...
    this._credencialService.guardarColaborador(this.codigo_colaborador ,CREDENCIAL).subscribe(data => {
      Swal.fire({
        position: 'center',
        icon: 'success',
      ...
      //this.toast.success({detail:"Contraseña Actualizada", duration:3000});
      setTimeout(() => {
        if (this.departamento == ddhh){
          this.router.navigate([`/home-ddhh/${this.cod}`])
          this.toast.success({ detail: "Bienvenido", summary: "Portal de Colaboradores DDHH", duration: 5000 });
        }else if (this.puesto.match(gerente)){
          this.router.navigate([`/home-admin/${this.cod}`])
          this.toast.success({ detail: "Bienvenido", summary: "Portal de Colaboradores DDHH", duration: 5000 });
        }else{
        this.router.navigate([`/home/${this.cod}`])
        this.toast.success({ detail: "Bienvenido", summary: "Portal de Colaboradores DDHH", duration: 5000 });
      }
    }, 2000);
    }, error => {
  ...
  async obtenerColaborador(codigo_colaborador: number) {
    ...
    try {
      ...
      this.primerInicioSesion(this.primeraVez);
...
obtenerImagen(){
  ...
  primerInicioSesion(contra: string){
    if (contra === "Ladylee.123"){
      this.primerSesion = true;
    }else{
      this.primerSesion = false;
    }
  }
